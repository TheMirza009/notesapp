...
class _AnimatedRightPanelState extends State<AnimatedRightPanel>
  ...
  void didUpdateWidget(AnimatedRightPanel oldWidget) {
    ...
    final wasNull = oldWidget.selectedChat == null;
    final isNull = widget.selectedChat == null;

    // Only animate when toggling between null and non-null
    if (wasNull == isNull) {
      _currentChat = widget.selectedChat;
      return;
    }
  ...
  Widget build(BuildContext context) {
    // ChatScreen is ALWAYS rendered when non-null — no animation wrapper
    if (_currentChat != null) {
      return widget.chatScreen;
    }

    // Only the placeholder fades
          ...
          child: Column(
            ...
            children: [
              SizedBox(
                ...
                child: NothingToSee(path: IconPaths.selectChat,)),
              const SizedBox(height: 16),
              Text(
                "Select a chat to view notes",
                style: TextStyle(
                  fontSize: 16,
