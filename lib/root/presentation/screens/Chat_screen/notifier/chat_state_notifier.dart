...
import 'package:notesapp/root/presentation/screens/Chat_Detail/screens/chat_media_screen.dart';
import 'package:notesapp/root/presentation/screens/Chat_Forward/chat_forward_screen.dart';
import 'package:notesapp/root/presentation/screens/Chat_screen/widgets/wrappers/anchor_wrapper.dart';
...
import 'package:notesapp/root/presentation/widgets/photo_view/media_preview_modal.dart';
...
class ChatStateNotifier extends Notifier<ChatState> {
  ...
  int visibleCount = 20,
  int batchSize = 50, // ✅ Larger batches for background
  int concurrency = 6, // ✅ More concurrent loads
  Duration delay = const Duration(milliseconds: 50), // ✅ Faster loading
}) async {
  ...
  state = state.copyWith(
    ...
    isLoading: false,
      ...
      if (messageIds.length > actualVisibleCount) {
        final remainingIds = messageIds.skip(actualVisibleCount).toList();
...
Future<void> _loadRemainingMessagesSilently(
  ...
  for (int i = 0; i < remainingIds.length; i += batchSize) {
    ...
    debugPrint('🔇 Silent batch ${i ~/ batchSize + 1}: +${batchMessages.length} messages (total: ${allMessages.length})');
  }
  ...
  int batchSize = 20,
  int concurrency = 4,
  Duration delay = const Duration(milliseconds: 150), // ✅ Slightly longer delay
}) async {
  for (int i = 0; i < remainingIds.length; i += batchSize) {
    if (_chat == null) return;
    ...
    if (prependMode && itemScrollController.isAttached) {
      final positions = itemPositionsListener.itemPositions.value;
      if (positions.isNotEmpty) {
        currentFirstVisibleIndex = positions
    ...
    final int batchLength = batchMessages.length;
    ...
    // ✅ Restore scroll position after prepend to prevent jump
    if (prependMode && currentFirstVisibleIndex != null && itemScrollController.isAttached) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (itemScrollController.isAttached) {
          // Adjust index by number of prepended messages
          final newIndex = currentFirstVisibleIndex! + batchLength;
          itemScrollController.jumpTo(index: newIndex);
        }
      });
    }

    debugPrint('📦 Batch ${i ~/ batchSize + 1}: ${batchMessages.length} messages ${prependMode ? "prepended" : "appended"}');
  }
}
