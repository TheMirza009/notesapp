...
//     await _isar.writeTxn(() async {                                         // Use managed objects inside txn
//       final managedMsg = await _isar.messages.get(message.isarId);          // Fetch the managed message by isarId first
//       if (managedMsg != null) {
//         await _isar.messages.delete(managedMsg.isarId);                     // Delete the message record
//       }

//       if (_chat != null) {
//         final managedChat = await _isar.chats.get(_chat!.isarID);           // Work on a managed chat instance
//         if (managedChat != null) {
//           await managedChat.messages.load();
//            // Try to find a managed message in the links (by isarId)
//           final linkIndex = managedChat.messages .where((m) => m.isarId == message.isarId) .toList();

          
//           if (linkIndex.isNotEmpty) {                                       // Remove by reference if found
//             for (final linked in linkIndex) {                               // remove each found occurrence
//               managedChat.messages.remove(linked);
//             }
//             await managedChat.messages.save();
//           }
...
//     // After deletion, handle media cleanup outside txn
//     final mediaObj = message.media.value;
//     final isMedia = mediaObj != null && mediaObj.type != Mediatype.text;

//     if (isMedia) {
//       final allMessages = await _isar.messages.where().findAll();     // Fetch all messages that have a media link
...
//       // Check if this media path is shared by others
//       final isUsedByMultiple = allMessages.hasDuplicateMediaPath(message);
//       if (!isUsedByMultiple) {
//         await MediaHandler.deleteMedia(mediaObj);                 // Only delete if no other message uses the same file
//       }
//     }
